...
4. Run the correct verification tier.
   Use `scripts/verify-fast.ps1` for normal code/config/doc automation changes.
   Use `scripts/verify-full.ps1` before any push and for user-facing flows, persistence, scripts, CI, hooks, auth, publishing, generation, or review changes.
5. Report evidence.
...
- Push feature branches and use PR-ready changes with passing quality checks.
...
- the required verification tier passed
