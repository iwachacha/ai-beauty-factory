...
export default function SettingsPage() {
  ...
  return (
    <FactoryShell title="Settings" subtitle="Store the API base URL and manage MCP/API access keys for your personal factory.">
      <section style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setApiBase(draftApiBase)} style={primaryButtonStyle}>Save</button>
        </div>
      </section>
      ...
      <section style={cardStyle}>
        ...
        <form onSubmit={createKey} style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
          <input value={name} onChange={event => setName(event.target.value)} placeholder="New key name" style={{ ...fieldStyle, flex: 1 }} />
          <button type="submit" style={primaryButtonStyle}>Create key</button>
        </form>
        {lastCreatedKey && <div style={{ ...softCardStyle, marginBottom: 16 }}>New key: <code>{lastCreatedKey}</code></div>}
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          {apiKeys.length === 0 && <div style={softCardStyle}>No API keys yet.</div>}
        </div>
      </section>
    </FactoryShell>
  )
